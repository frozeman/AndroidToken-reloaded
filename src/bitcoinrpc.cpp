...
// androidtoken: get network Gh/s estimate
...
Value setaccount(const Array& params, bool fHelp)
{
    ...
    if (!address.IsValid())
        throw JSONRPCError(-5, "Invalid androidtoken address");
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(-5, string("Invalid androidtoken address:")+s.name_);
...
Value walletpassphrase(const Array& params, bool fHelp)
{
    ...
    // androidtoken: if user OS account compromised prevent trivial sendmoney commands
    if (params.size() > 2)
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "wallet encrypted; androidtoken server stopping, restart to run with encrypted wallet";
}
...
// androidtoken: get information of sync-checkpoint
...
// androidtoken: reserve balance from being staked for network protection
...
// androidtoken: check wallet integrity
...
// androidtoken: make a public-private key pair
