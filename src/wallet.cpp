...
// androidtoken: optional setting to unlock wallet for block minting only;
...
// androidtoken: total coins staked (non-spendable until maturity)
       ...
       BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
       {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                if (pcoin->nTime > nSpendTime)
                    continue;  // androidtoken: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            loop
            {
                ...
                // androidtoken: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    if (!GetBoolArg("-avatar")) // androidtoken: not avatar mode
                    {
...
// androidtoken: create coin stake transaction
...
// androidtoken: check 'spent' consistency between wallet and txindex
// androidtoken: fix wallet spent state according to txindex
...
// androidtoken: disable transaction (only for coinstake)
