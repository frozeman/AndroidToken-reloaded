...
{
    // Wait for IRC to get it first - disabled with androidtoken
...
// testnet dns seed begins with 't', all else are androidtoken dns seeds.
static const char *strDNSSeed[][2] = {
    {"",""}
    // {"seed", "seed.androidtoken.net"},
    // {"seedppc", "seedppc.androidtoken.net"},
    // {"altcointech", "dnsseed.ppc.altcointech.net"},
    // {"tnseed", "tnseed.androidtoken.net"},
    // {"tnseedppc", "tnseedppc.androidtoken.net"},
};
...
void ThreadDNSAddressSeed2(void* parg)
{
    ...
    if (true /*!fTestNet*/)  // androidtoken enables dns seeding with testnet too
    {
...
// androidtoken: stake minter thread
...
bool BindListenport(string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, sizeof(sockaddr)) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to port %d on this computer.  AndroidToken is probably already running."), ntohs(sockaddr.sin_port));
        else
...
void StartNode(void* parg)
{
    ...
    // IRC disabled with androidtoken
    ...
    // androidtoken: mint proof-of-stake blocks in the background
